...
{	
	...
	public class FlxBasic
	{
		...
		public function logMe(indent:int):void 
		{
			var str:String = "";
			for (var i:int = 0; i < indent; i++) str += " ";
			FlxG.log(str + "+ " + this.toString());
		}
		...
		{
			var opt:Array = [];
			if ( exists ) opt.push("E");
			if ( active ) opt.push("A");
			if ( alive ) opt.push("L");
			if ( visible ) opt.push("V");
			return FlxU.getClassName(this,true) + " (" + opt.join("/") + ")";
		}
	}
}
